...
export class AuthResolver {
  ...
  async authLogin(
    ...
    @Args('data') args: AuthLoginInput
  ) {
    const user = await this.getUserByUsername(args.name, ctx.prisma);
    ...
    if (!correctPassword)
      throw new HttpException(ApiError.AuthLogin.INCORRECT_PASSWORD, 400);
  ...
  async authPasswordResetRequest(
    ...
    if (possibleUsers.length === 0)
      throw new HttpException(
        ApiError.AuthPasswordResetRequest.USER_NOT_FOUND,
        400
      );
  ...
  async authPasswordResetConfirmation(
    ...
    try {
      ...
      throw new HttpException(
        ApiError.AuthPasswordResetConfirmation.UNAUTHORIZED,
        400
      );
    }
    ...
    if (!user)
      throw new HttpException(
        ApiError.AuthPasswordResetConfirmation.USER_NOT_FOUND,
        400
      );
  ...
  async authRegister(
    ...
    if (!this.config.publicRegistration)
      throw new HttpException(
        ApiError.AuthRegister.NO_PUBLIC_REGISTRATIONS,
        403
      );

    if (await this.getUserByUsername(args.name, ctx.prisma))
  ...
  async authPasswordChange(
    ...
    if (!user)
      throw new HttpException(ApiError.AuthPasswordChange.USER_NOT_FOUND, 400);
    ...
    if (!correctPassword)
      throw new HttpException(ApiError.AuthPasswordChange.WRONG_PASSWORD, 400);
